...
function taskListConfigSerialise() {
        var model = jQuery("#backlog_list").jqGrid('getGridParam', 'colModel');
...
function change_group() {
  ...
  if(vl) {
    group_value = vl;
      ...
      if(vl == "clear") {
        jQuery("#backlog_list").jqGrid('groupingRemove',true);
      } else {
        jQuery("#backlog_list").jqGrid('groupingGroupBy',vl);
      }
    });
  }
}
...
function initTaskList() {
  ...
  jQuery("#backlog_list").jqGrid('sortableRows', {
  ...
  jQuery("#backlog_list").jqGrid('gridResize', {
...
//  });
  ...
  jQuery('#backlog_pager_center').remove();
  resizeGrid();
}
